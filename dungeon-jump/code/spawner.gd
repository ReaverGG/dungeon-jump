...
func _process(delta: float) -> void:
	speed_multiplier += delta / 100
	
	if player:
		if global_position.y + tile_distance * tiles_ahead > player.global_position.y:
...
func get_window_size(tile_object) -> Vector2:
